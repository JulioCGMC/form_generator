...
class _FakeTextFieldState extends State<FakeTextField> {
  ...
  Future<void> _showPicker(context) async {
    ...
    if (widget.value.fieldType == FieldType.date) {
      DateTime initialDate = widget.value.initialValue != null 
        ? widget.value.initialValue is DateTime 
          ? widget.value.initialValue : DateTime.tryParse(widget.value.initialValue)
      : DateTime.now();
      ...
      TimeOfDay initialTime = widget.value.initialValue != null 
        ? widget.value.initialValue is TimeOfDay 
          ? widget.value.initialValue : widget.value.initialValue is DateTime 
            ? TimeOfDay.fromDateTime(widget.value.initialValue) 
            : TimeOfDay.fromDateTime(DateTime.tryParse(widget.value.initialValue) ?? DateTime.now())
      : TimeOfDay.now();
    ...
    if (result != null) {
      if (widget.value.fieldType == FieldType.date) {
        ...
        setState(() { widget.controller.text = result.toString().split(" ")[0]; });
      }
      else {
        widget.onChanged!(result.format(context));
        setState(() { widget.controller.text = result.format(context); });
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return TextFormField(
      ...
      focusNode: widget.value.focusNode ?? FocusNode(debugLabel: widget.value.id),
